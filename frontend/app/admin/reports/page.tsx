...
export default function ReportsPage() {
      ...
      try {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          if (i > 0) {
            let prev = data[i - 1];
            if (prev.revenue > 0) {
              growth = ((current.revenue - prev.revenue) / prev.revenue) * 100;
            }
          }
          
          let [year, month] = current.period.split('-');
          let date = new Date(parseInt(year), parseInt(month) - 1);
          let formattedMonth = date.toLocaleDateString('en-US', {month: 'short', year: '2-digit'});
        ...
        // Take last 12 months for display to match the 12-Month Performance label
        setMonthlyData(transformed.slice(-12));
      } catch (error) {
  ...
  const total12mRides   = monthlyData.reduce((a,r)=>a+r.rides,0).toLocaleString();
  const avgRevenuePerRide = monthlyData.length ? (monthlyData.reduce((a,r)=>a+r.revenue,0) * 100000 / monthlyData.reduce((a,r)=>a+r.rides,0)).toFixed(1) : "0.0";
          ...
          <div className="grid md:grid-cols-3 gap-4">
            <div className="glass rounded-xl p-5">
              ...
              <div className="text-xs text-emerald-400 mt-1">
                {monthlyData.length > 1 && monthlyData[monthlyData.length-1].growth !== null ? 
                  `${monthlyData[monthlyData.length-1].growth > 0 ? '↑' : '↓'} ${Math.abs(monthlyData[monthlyData.length-1].growth)}% vs prior month` : 
                  '—'}
              </div>
            </div>
            <div className="glass rounded-xl p-5">
            ...
            <div className="glass rounded-xl p-5">
              ...
              <div className="text-3xl font-display font-bold text-white">₹{avgRevenuePerRide}</div>
