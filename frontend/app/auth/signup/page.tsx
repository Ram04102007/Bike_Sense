...
import { Bike, Building2, ChevronRight, ArrowLeft, Loader2 } from "lucide-react";
...
export default function SignUpPage() {
  ...
  const [role, setRole] = useState<"admin"|"consumer"|null>((params.get("role") as any) || null);
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.status === "complete") {
        ...
        toast.success("Welcome to BikeSense! 🚲");
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-8">
        <div className="w-full max-w-md">
          ...
          <div className="flex items-center gap-2 mb-8 lg:hidden">
            ...
            <span className="font-display font-bold text-white">Bike<span className="text-brand-400">Sense</span></span>
          </div>
            ...
            <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}}>
              ...
              <div className="space-y-4">
                {[
                  ...
                  <button key={r} onClick={()=>{setRole(r as any);setStep("form");}}
          ...
          {step === "form" && (
            <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}}>
              ...
              <p className="text-center text-sm text-slate-500 mt-4">
                Have an account? <Link href="/auth/login" className="text-brand-400 hover:text-brand-300">Sign in</Link>
              </p>
            </motion.div>
          )}
          ...
          {step === "verify" && (
            <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}}>
              ...
              <p className="text-slate-400 mb-6">We sent a 6-digit code to <strong className="text-white">{form.email}</strong></p>
