    ...
    <motion.div layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
      ...
      <div className="p-5">
        ...
        <div className="flex flex-wrap gap-1.5 mb-3">
          ...
          <span className={`text-xs px-2 py-0.5 rounded-full ${bike.available ? (bike.available_count < 5 ? "bg-amber-500/10 text-amber-400 border border-amber-500/20" : "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20") : "bg-red-500/10 text-red-400 border border-red-500/20"}`}>
            {bike.available ? (bike.available_count < 5 ? `Only ${bike.available_count} left` : `${bike.available_count} available`) : "Out of stock"}
          </span>
        </div>
